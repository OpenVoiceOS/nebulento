...
Nebulento ships a comparative accuracy and speed benchmark in `benchmark/compare.py`. It runs on two OpenVoiceOS evaluation datasets and reports every engine, and every nebulento `MatchStrategy`, side by side. Per-engine threshold calibration makes the numbers reflect each engine's real ceiling, not just its shipped default.
...
- **False negative**: no intent fires. OVOS hands the utterance to its fallback chain: common-query, the LLM fallback, online search. These exist to handle "I don't know what you meant." Worst case, the user re-phrases. Best case, the LLM gets it right.

The cost ratio is roughly 5-10x in favor of false negatives. F1, which weights precision and recall equally, is the wrong summary metric. **F_β with β=0.5 weights precision twice as much as recall** and is the right summary for OVOS.
...
| `nebulento-hierarchical` (2 variants) | **subject** | two-stage domain-to-intent routing. Tested with `damerau` (gate=0.0) and `token-set-ratio` (gate=0.7) |
...
- `TOKEN_SORT_RATIO`, `TOKEN_SET_RATIO`: alphabetised-token and set-overlap variants of `RATIO`
...
2. **All four `partial-*` token strategies are penalised hard.** They reward any substring overlap. On a 50-intent corpus where many intents share common verbs (`set`, `play`, `cancel`, `read`), this means cross-domain confusions multiply. `partial-token-ratio` and `partial-token-set-ratio` produce so many false positives that **they never reach 99% precision at any threshold** (R@P99 = 0.0%).
...
The flip side: damerau is **slower** than the ratio variants when slot expansion balloons the candidate set (7.5 ms median vs 3.1 ms for `ratio`). For a typical OVOS install that is irrelevant. For a large skill catalogue, it is worth knowing.
...
If anything, the data suggests a default *threshold* tweak: dropping from 0.50 to 0.43 lifts F_0.5 from 0.909 to 0.918, trading a few extra FPs (17 to 26) for higher recall. The current default is conservative, which is appropriate for OVOS, where FPs hurt more.
...
- FPs collapse from 26 to **11** (lowest of any nebulento variant in the table).
...
In other words, the `domain_threshold` gate is a **precision boost at the cost of recall**. It earns its keep when off-topic rejection matters more than catch rate, for example in a deployment with strong LLM fallback that can absorb the missed matches.
...
- **Latency favours hierarchical** modestly. Each utterance is only scored against the intents in the winning domain. The `token-set-ratio` variant is **~2x faster** than the flat default.
...
- **F1** = 2·P·R / (P + R)
- **F_0.5** = 1.25·P·R / (0.25·P + R), weights precision 2x recall (default summary metric for OVOS)
